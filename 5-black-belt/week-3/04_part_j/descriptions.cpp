...
void Descriptions::PrepRender(
	...
	if (stops_.size() == 1) {
		...
		double x_step = (width - 2 * padding) / (stops_.size() - 1);
		for (size_t i = 0; i < stops_.size(); ++i) {
			stops_[i]->svg_pos.x = i * x_step + padding;
		}

		sort(begin(stops_), end(stops_),
			[](const Stop* lhs, const Stop* rhs)
			{ return lhs->sphere_pos.latitude < rhs->sphere_pos.latitude; });
		double y_step = (height - 2 * padding) / (stops_.size() - 1);
		for (size_t i = 0; i < stops_.size(); ++i) {
			stops_[i]->svg_pos.y = height - padding - i * y_step;
		}
	}
}
